...
import { ArrowRight, Building2, LayoutGrid, List, Music } from 'lucide-react'
...
export default function LabelsPage() {
  ...
  // Apply the text search and first-letter filters together.
  const normalizedQuery = searchQuery.trim().toLowerCase()
  const filteredLabels = labels.filter(label => {
    const matchesSearch =
      label.name.toLowerCase().includes(normalizedQuery) ||
      (label.bio || '').toLowerCase().includes(normalizedQuery)
    const matchesLetter =
      selectedLetter === null ||
  ...
  return (
    <CartProvider>
      <div className="min-h-screen bg-background text-foreground">
        ...
        <main className="pt-24 pb-16 min-h-[70vh] relative overflow-hidden">
          {/* Ambient Glow background */}
          <div className="absolute top-20 left-1/4 w-96 h-96 bg-primary/5 rounded-full blur-[100px] pointer-events-none -z-10" />
          <div className="absolute bottom-20 right-1/4 w-96 h-96 bg-amber-500/5 rounded-full blur-[120px] pointer-events-none -z-10" />

          <section className="py-12 md:py-16">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">

              {/* Header and Search section bar */}
              <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-8 border-b border-border/20 pb-8">
                <div className="space-y-4">
                  <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full border border-primary/20 bg-primary/5 text-primary text-[10px] font-bold uppercase tracking-widest">
                  ...
                  <p className="text-muted-foreground text-sm md:text-base max-w-xl leading-relaxed">
                    Explore independent record labels on artistrax. Discover curated releases, label merchandise, and subscribing directly to back catalogs.
                  </p>
                </div>
                ...
                <div className="w-full md:w-80 relative">
                  <input
                    ...
                    className="w-full h-11 px-4 py-2 bg-card border border-primary/20 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary/45 text-foreground placeholder:text-muted-foreground/60 transition-all duration-300"
                  />
                  {searchQuery && (
                    <button
              ...
              {/* Letter filter and view controls */}
              <div className="mb-8 flex flex-col gap-4 border-b border-border pb-5 lg:flex-row lg:items-center lg:justify-between">
                <div className="flex gap-1.5 overflow-x-auto pb-1" aria-label="Filter labels by first letter">
                  <button
                    type="button"
                    onClick={() => setSelectedLetter(null)}
                    className={`shrink-0 rounded-sm border px-3 py-1.5 text-xs font-medium transition-colors hover:bg-accent/10 ${
                      selectedLetter === null
                  ...
                  {LETTERS.map(letter => (
                    <button
                      ...
                      className={`h-8 w-8 shrink-0 rounded-sm border text-xs font-medium transition-colors hover:bg-accent/10 ${
                ...
                <div className="flex shrink-0 items-center gap-1 rounded-sm border border-border p-1" aria-label="Choose label view">
                  <button
                    type="button"
                    onClick={() => setViewMode('grid')}
                    className={`inline-flex items-center gap-2 rounded-sm px-3 py-1.5 text-xs font-medium transition-colors ${
                      viewMode === 'grid'
              ...
              {loading ? (
                  ...
                  <div className="border-t border-border">
                    {filteredLabels.map(label => (
                      ...
                      >
                        ...
                        <div className="min-w-0 flex-1 sm:grid sm:grid-cols-[minmax(10rem,0.35fr)_1fr] sm:items-center sm:gap-6">
                ...
                <div className="text-center py-24 bg-card/40 border border-primary/15 rounded-3xl p-8 glass-panel max-w-md mx-auto">
                  ...
                  <p className="text-muted-foreground text-xs">
                    No record labels match the selected filters.
                  </p>
                </div>
              )}
            </div>
